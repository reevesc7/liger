...
def _cmd_local_run(args: Namespace) -> None:
    from liger.run import tpot
...
def _cmd_slurm_run(args: Namespace) -> None:
    from liger.run import tpot
    checkpoint_dir = Path(args.checkpoint)
    if not checkpoint_dir.is_dir():
        raise FileNotFoundError(f"{checkpoint_dir!r} is not a directory")
...
def _init_init_parser(tpot_init_parser: ArgumentParser) -> None:
    ...
    tpot_init_parser.set_defaults(func=_cmd_init)


def _init_run_parser(tpot_run_parser: ArgumentParser) -> None:
    ...
    local_run_parser.set_defaults(func=_cmd_local_run)
    ...
    slurm_run_parser.set_defaults(func=_cmd_slurm_run)


def init_tpot_parser(tpot_parser: ArgumentParser) -> None:
    subparsers = tpot_parser.add_subparsers(dest="command", required=True)
    _init_init_parser(subparsers.add_parser(
        _RunCommand.INIT,
        description="Initialize one or more TPOT run output/checkpoint directories "
            "from a config.",
    ))
    _init_run_parser(subparsers.add_parser(
        _RunCommand.RUN,
